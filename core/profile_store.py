"""Profile storage and CRUD helpers for YAML-backed domain profiles.

This module keeps file-system concerns out of request handlers:
- stable/safe profile id generation from labels
- input validation + normalization
- atomic create/update writes
- profile listing with metadata for UI ordering
"""
...
def create_profile(payload: dict, profiles_dir: Path | None = None) -> dict:
    ...
    profile_id = validate_profile_id(requested_id) if requested_id else generate_profile_id(str(payload.get("label") or ""), directory)
